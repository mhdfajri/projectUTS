...
<body>
<?php foreach($db -> viewupdatefilm($_GET['title']) as $a){ ?>
    <div class="center">
    <h1>Edit film</h1>
        <form action="../controller/process.php?action=updatefilm" method="post" enctype="multipart/form-data">
        <input type="hidden" class="input1" name="no" value="<?php echo $a['no'] ?>">
            <input type="text" class="input1" name="title" value="<?php echo $a['title'] ?>">
            <label class= "text_field">title</label>
            <textarea class="input2" name="sinopsis"><?php echo$a['sinopsis'] ?></textarea>
